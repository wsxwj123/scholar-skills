...
> 🔴 **断线重连保护（只看 phase 会白跑一整轮检索）：** `set-phase 1.6` 是**进入 Phase 1.6 时**才执行的，所以"用户已确认选题方向、但会话在进 1.6 之前断了"这个窗口里 `phase` 仍是 1.5。此时若照上面只看 phase，会把探索性检索**从头重跑一遍**（串行 ≥1s、烧额度、结果和上次一样）。
> **判据用产物、不用 phase：** 先 Read `data/research_gap.json`，**若已有任一条目带 `"selected": true`**（第 5 步落盘的用户确认证据）→ 本阶段已完成，跳过步骤 0–4，补跑 `set-phase --phase 1.6` 后直接进 Phase 1.6。没有该标记才从步骤 0 开始。
